...
/**
 * {@link UserSettingsRepository} 통합 테스트 — Flyway V3 스키마와 엔티티 매핑 실연동 검증.
 */
class UserSettingsRepositoryTest extends RepositoryTestBase {
    ...
    void 설정을_저장하고_소유자로_조회한다() {
        ...
        userSettingsRepository.save(UserSettings.create(owner, "081", 0.5, "standard"));
        ...
        assertThat(userSettingsRepository.findByOwner_Id(owner.getId()))
                ...
                .satisfies(s -> {
                    ...
                    assertThat(s.getDisplayMode()).isEqualTo("standard");
                });
    }
    ...
    void 주거래_은행을_비워도_저장하고_조회할_수_있다() {
        ...
        userSettingsRepository.save(UserSettings.create(owner, null, 0.0, "standard"));
