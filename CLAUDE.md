...
FxAtlas는 외화 목표·환전 타이밍 제안 서비스이며, **이 레포(백엔드)가 계산 엔진을 전담**한다(몬테카를로 시뮬레이션, 안전/기회 버킷 분리, 변동성·집중도 계산 등). 프론트는 API 응답을 표시만 하고 재계산하지 않는다.
...
2. **현재 브랜치 성격과 요청 작업 유형이 다르면 새 브랜치 생성을 먼저 제안**한다. 브랜치명은 `feat/<scope>` · `fix/<scope>` · `chore/<scope>`.
